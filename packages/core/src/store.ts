...
import { getPath, removeKey, setKey, setPath } from './state-utils'
...
> {
	...
	validateFields(): boolean {
		...
		Object.entries(this.state.validators).forEach(([name, validate]) => {
			...
			this.state = {
				...this.state,
				errors: setKey(this.state.errors, name, validate?.(value) || null),
			}
		})
		this.notify()
	...
	validateField(name: string): boolean {
		...
		this.state = {
			...this.state,
			errors: setKey(this.state.errors, name, error),
		}
		this.notify()
